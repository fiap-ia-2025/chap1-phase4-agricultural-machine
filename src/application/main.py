...
def main_menu():
    ...
    import os, sys
    conn = create_database()          # Conexão única para todo o ciclo

    weather_api = None                # Só instanciamos se necessário
    ...
    while True:
        ...
        if choice == '1':
            ...
            # Buscar a chave de API e instancia a integração
            if weather_api is None:                     # evita recriar sempre
                api_key = os.getenv("WEATHER_API_KEY")
                if not api_key:
                    print("\n⚠️  ERRO: variável de ambiente WEATHER_API_KEY não definida.")
                    input("Pressione Enter para voltar ao menu...")  # mantém fluxo
                    continue
                try:
                    weather_api = WeatherIntegration(api_key=api_key)
                except Exception as e:
                    print(f"\n⚠️  ERRO ao inicializar integração de clima: {e}")
                    weather_api = None  # garante que tentará de novo da próxima vez
            ...
            try:
                ...
                print(f"\n⚠️  ERRO durante a consulta de clima: {e}")
                input("Pressione Enter para voltar ao menu...")
